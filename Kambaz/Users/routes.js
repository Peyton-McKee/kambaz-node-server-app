...
export default function UserRoutes(app) {
  ...
  const findCoursesForUser = (req, res) => {
    let { userId, enrolled } = req.params;
    if (userId === "current") {
      const currentUser = req.session["currentUser"];
      if (!currentUser) {
        res.sendStatus(401);
    ...
    const courses = courseDao.findCoursesForUser(userId, enrolled === "true");
    res.json(courses);
  };
  ...
  const getEnrollmentsForUser = (req, res) => {
    const currentUser = req.session["currentUser"];
    const enrollments = enrollmentsDao.getEnrollmentsForUser(currentUser._id);
